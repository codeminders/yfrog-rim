...
public class HomeListScreen extends TopScreenBase
    implements NetworkListener, ImageDataListener
{

    private static HomeListScreen _instance = null;

    public static synchronized HomeListScreen getInstance() {
        if (_instance == null)
            _instance = new HomeListScreen();
        return _instance;
    }
    ...
    protected void makeFormMenu(Menu menu) {
        ...
        mi = new MenuItem(getRes(), MENU_NEW_TWEET, 1000, 10) {
            public void run() { newTweet(); }
        };
        menu.add(mi);
        def = mi;
    ...
    public void refreshData() {
        _list.clear();
        _loadRequestID = TwitterManager.getInstance().statusesFriendsTimeline();
    ...
    private void loadMore() {
        ...
        if (maxID < 0)
            ...
            _loadRequestID = TwitterManager.getInstance().statusesFriendsTimeline(maxID);
